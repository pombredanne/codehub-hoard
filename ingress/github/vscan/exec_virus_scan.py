...
def _process_vscan_output(output, ref_path):
    if output is None:
        return None
...
def _execute_vscan(cloned_project_path, ref_path):
    ...
    report = _process_vscan_output(output, ref_path)
    return report
...
def _process_messages(config, messages):
    for message in messages:
        if message is not None:
            ...
            vscan_result = _execute_vscan(cloned_project_path, ref_path)
